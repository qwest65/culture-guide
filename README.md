...
|---|---|
| `dev` | тесты и сборка, затем GitHub Release `v<версия>` с единственным файлом `CultureGuide.apk` и `SHA256SUMS.txt`, помеченный как Latest, и отдельный релиз `karavan` с `MalenkiyKaravan.apk` |
...
Постоянная ссылка на «Маленький караван»: https://github.com/qwest65/culture-guide/releases/download/karavan/MalenkiyKaravan.apk. Релиз `karavan` пересоздаётся при каждой сборке из `dev`, поэтому ссылка всегда ведёт на свежую версию.

Чтобы выпустить новую версию, поднимите `versionName` и `versionCode` в [`app/build.gradle.kts`](app/build.gradle.kts) (для «Маленького каравана» — в [`app-kids/build.gradle.kts`](app-kids/build.gradle.kts)) и влейте изменения в `dev`. Для работы CI в настройках репозитория нужен secret `MAPKIT_API_KEY`.
